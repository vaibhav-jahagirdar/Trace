...
}).refine((value) => Boolean(value.password || value.securityAnswer), { message: "Enter your password or the organization security answer" });

export type CreateOrganizationInput = z.infer<typeof createOrganizationSchema>;
export type UpdateOrganizationInput = z.infer<typeof updateOrganizationSchema>;
export const updateMemberRoleSchema = z.object({
  role: z.enum(["RECRUITING_ADMIN", "RECRUITER", "HIRING_MANAGER", "INTERVIEWER", "VIEWER"]),
});
export const membershipIdParamSchema = orgIdParamSchema.extend({ membershipId: z.string().uuid() });
export const addMemberSchema = z.object({ email: z.string().email().trim().toLowerCase(), role: z.enum(["RECRUITING_ADMIN", "RECRUITER", "HIRING_MANAGER", "INTERVIEWER", "VIEWER"]) });
export const ownershipTransferSchema = z.object({ targetMembershipId: z.string().uuid(), password: z.string().min(1) });
