...
export async function createJobEligibilityCriteriaRecord(eligibilityCriteriaData: JobEligibilityCriteriaInput, jobId: string, client: PoolClient) {
    ...
    const eligibilityCriteriaResult = await client.query(
        `INSERT INTO job_eligibility_criteria
         ...
         RETURNING id `,
         [jobId, currency, salary_min, salary_max, experience_min_years, experience_ideal_years, experience_max_years, notice_period_ideal_days, notice_period_max_days, relocation_assistance, visa_sponsorship, work_authorization_required, minimum_education_level]
    );
